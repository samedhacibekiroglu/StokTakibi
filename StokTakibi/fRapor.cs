...
namespace StokTakibi
{
    public partial class fRapor : Form
    {
        ...
        public void bGoster_Click(object sender, EventArgs e)
        {
            ...
            {
                if (listFitrelemeTuru.SelectedIndex == 0) //Tümünü getir
                {
                    ...
                    tSatisNakit.Text = Convert.ToDouble(islemozet.Where(x => x.Iade == false && x.Gelir == false && x.Gider == false).Sum(s => s.Kart)).ToString("C2");
                    ...
                    tIadeNakit.Text = Convert.ToDouble(islemozet.Where(x => x.Gelir == true).Sum(x => x.Nakit)).ToString("C2");
                    tIadeKart.Text = Convert.ToDouble(islemozet.Where(x => x.Gelir == true).Sum(x => x.Kart)).ToString("C2");
                    ...
                    double kdbtutarlarisatis = Islemler.DoubleYap(satistablosu.Where(x => x.Iade == false).Sum(x => x.KdvTutari).ToString());
                    double kdbtutarlariiade = Islemler.DoubleYap(satistablosu.Where(x => x.Iade == true).Sum(x => x.KdvTutari).ToString());
                    tKdvToplam.Text = (kdbtutarlarisatis - kdbtutarlariiade).ToString("C2");
                }
                else if (listFitrelemeTuru.SelectedIndex == 1) //Satışlar
                {
                    db.IslemOzet.Where(x => x.Tarih >= baslangic && x.Tarih <= bitis && x.Iade == false && x.Gelir == false && x.Gider == false).Load();
                    var islemozet = db.IslemOzet.Local.ToBindingList();
                    gridListe.DataSource = islemozet;
                }
                else if (true) //İadeler
                {
        ...
        {
            listFitrelemeTuru.SelectedIndex = 0;
