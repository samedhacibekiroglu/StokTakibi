   ...
   static class Raporlar
    {
        ...
        public static string TarihBslangic { get; set; }
        ...
        public static void RaporSayfasiRaporu(DataGridView dgv)
        {
            ...
            f.reportViewer1.LocalReport.ReportPath=Application.StartupPath + @"\rpGenelRapor.redlc";

            ReportParameter[] prm = new ReportParameter[13];
            prm[0] = new ReportParameter("Baslik", Baslik);
            prm[1] = new ReportParameter("TarihBaslangic", TarihBslangic);
            prm[2] = new ReportParameter("TarihBitis", TarihBitis);
            prm[3] = new ReportParameter("SatisKart", SatisKart);
            prm[4] = new ReportParameter("IadeNakit", IadeNakit);
            prm[5] = new ReportParameter("IadeKart", IadeKart);
            prm[6] = new ReportParameter("GelirKart", GelirKart);
            prm[7] = new ReportParameter("GelirNakit", GelirNakit);
            prm[8] = new ReportParameter("GiderKart", GiderKart);
            prm[9] = new ReportParameter("GiderNakit", GiderNakit);
            prm[10] = new ReportParameter("SatisNakit", SatisNakit);
        ...
        public static void StokRaporu(DataGridView dgv)
        {
            ...
            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                list.Add(new Urun
                {
                    ...
                    Miktar=Islemler.DoubleYap(dgv.Rows[i].Cells["Aciklama"].Value.ToString()),
                    Aciklama = dgv.Rows[i].Cells["Aciklama"].Value.ToString()
            ...
            f.reportViewer1.LocalReport.DataSources.Clear();
            f.reportViewer1.LocalReport.DataSources.Add(rs);
            f.reportViewer1.LocalReport.ReportPath = Application.StartupPath + @"\rpStokUrun.redlc";
        ...
        internal static void StokIzlemeRaporu(gridOzel gridListe)
        {
            throw new NotImplementedException();
        }

        internal static void StokRaporu(gridOzel gridListe)
        {
            throw new NotImplementedException();
        }
        ...
        public static void StokIzlemeRaporu(DataGridView dgv)
        {
            ...
            for (int i = 0; i < dgv.Rows.Count; i++)
            {
                list.Add(new StokHareket
                {
                    ...
                    UrunAd = dgv.Rows[i].Cells["UrunAd"].Value.ToString(),
                    Birim = dgv.Rows[i].Cells["Birim"].Value.ToString(),
                    UrunGrup = dgv.Rows[i].Cells["ÜrünGrup"].Value.ToString(),
                    Miktar = Islemler.DoubleYap(dgv.Rows[i].Cells["Aciklama"].Value.ToString()),
                    //Aciklama = dgv.Rows[i].Cells["Aciklama"].Value.ToString(),
                    Kullanici = dgv.Rows[i].Cells["Kullanıcı"].Value.ToString(),
                    Tarih = Convert.ToDateTime(dgv.Rows[i].Cells["Kullanıcı"].Value.ToString())
                });
            }
            ReportDataSource rs = new ReportDataSource();
            ...
            f.reportViewer1.LocalReport.DataSources.Clear();
            f.reportViewer1.LocalReport.DataSources.Add(rs);
            f.reportViewer1.LocalReport.ReportPath = Application.StartupPath + @"\rpStokIzleme.redlc";
        ...
        internal static void RaporSayfasiRaporu(gridOzel gridListe)
        {
            throw new NotImplementedException();
        }
    }
